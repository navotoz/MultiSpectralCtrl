...
def camera_model(camera: Camera):
    with camera as cam:
        model = cam.get_model()
...
class AlliedVisionCtrl:
    ...
    def __init__(self, logging_handlers: (list, tuple) = ()):
        self.__log = make_logger(f"AlliedVisionCtrl", handlers=logging_handlers)

        with Vimba.get_instance() as vimba:
            camera_list = vimba.get_all_cameras()
            if not camera_list:
                msg = f'AlliedVision cameras were not detected. Check if cameras are connected to USB3 via USB3 cable.'
                self.__log.critical(msg)
                raise RuntimeError(msg)
            else:
                for camera_device in camera_list:
                    with camera_device as cam:
                        ...
                        self.cameras_dict[camera_model(cam)] = cam
